...
int main(int argc, char *argv[]) {
    ...
    try {
		...
		std::cout << "Number of nireq" << FLAGS_nireq << std::endl;
		std::cout << "Number of threads" << FLAGS_nthreads << std::endl;
		std::cout << "Number of streams" << FLAGS_nstreams << std::endl;

		std::cout << FLAGS_m << FLAGS_d << std::endl;
		...
		std::string ola = FLAGS_m;
		std::string device = FLAGS_d;
		std::string var1 = ola.append(device);
		std::string teste = var1.append(".txt\\");

		size_t pos = 0;
		std::string token;
		std::string delimiter = "\\";
		while ((pos = teste.find(delimiter)) != std::string::npos) {
			token = teste.substr(0, pos);
			//std::cout << token << std::endl;
			teste.erase(0, pos + delimiter.length());
		}
        ...
        while ((niter != 0LL && iteration < niter) ||
			...
			auto execTime2 = std::chrono::duration_cast<ns>(Time::now() - timeNew).count();
			
			float time = execTime2 / 1000000.00;
		...
		std::vector<double> vec = inferRequestsQueue.getLatencies();
		std::cout << "Tamanho inferRequestQueue" << vec.size() << std::endl;
		...
		file << float_to_string(totalDuration) << std::endl;
		if (device_name.find("MULTI") == std::string::npos)
			file<< float_to_string(latency) << std::endl;
		file << float_to_string(fps) << std::endl;
